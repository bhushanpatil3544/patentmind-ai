...
def download_actual_patent_pdf(patent_id: str):
    ...
    or generates a comprehensive patent dossier report from vector store data.
    """
    ...
    # 2. Generate comprehensive patent dossier from vector store data
    report_lines = []
    report_lines.append("=" * 70)
    report_lines.append("PATENTMIND AI - PATENT SPECIFICATION DOSSIER REPORT")
    report_lines.append("=" * 70)
    report_lines.append(f"PATENT NUMBER : {clean_id}")
    report_lines.append(f"GENERATED ON  : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report_lines.append("")

    # Search vector store for all chunks belonging to this patent
    chunks_found = []
    try:
        if rag_chain and rag_chain.db:
            # Search with patent number as query to get relevant chunks
    ...
    if chunks_found:
        ...
        report_lines.append("")
        report_lines.append("-" * 70)
        report_lines.append("PATENT SECTIONS & CLAIMS")
        report_lines.append("-" * 70)
        report_lines.append("")

        for idx, chunk in enumerate(chunks_found, 1):
            meta = chunk.get("metadata", {})
            ...
            report_lines.append(f"--- Section {idx}: {section} ---")
            if claim_num:
                report_lines.append(f"Claim Number: #{claim_num}")
            report_lines.append(f"Relevance Score: {score:.4f}")
            report_lines.append("")
            report_lines.append(text)
            report_lines.append("")
    else:
        # Check relational DB for metadata
        try:
            patent_meta = relational_db.get_patent_meta(clean_id)
            if patent_meta:
                report_lines.append(f"TITLE         : {patent_meta.get('title', 'N/A')}")
                report_lines.append(f"ASSIGNEE      : {patent_meta.get('assignee', 'N/A')}")
                report_lines.append(f"FILING DATE   : {patent_meta.get('filing_date', 'N/A')}")
                report_lines.append("")
        except Exception:
            pass
        report_lines.append("Full patent text data is available in the local deployment.")
        report_lines.append(f"You can also view this patent at: https://patents.google.com/patent/{clean_id}/en")

    report_lines.append("")
    report_lines.append("=" * 70)
    report_lines.append("GENERATED BY PATENTMIND AI ENTERPRISE PLATFORM")
    report_lines.append(f"Regards, Bhushan Shelke")
    report_lines.append("=" * 70)
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="Patent_{clean_id}_Dossier.txt"'
        }
    )
