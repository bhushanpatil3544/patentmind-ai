...
export default function PatentDetailsView({ patentNumber, onBack, onCompare }) {
  const handleDownloadToDevice = () => {
    const targetPatent = patent || {
      number: patentNumber || 'US11740881B2',
      title: 'High Performance AI Hardware Accelerator Architecture',
      assignee: 'PatentMind Research Labs',
      inventors: 'Dr. Alex Vance, Maria Santos',
      filingDate: '2023-11-14',
      cpcCode: 'G06N 3/063',
      similarityScore: 98.4,
      infringementRiskScore: 12,
      abstract: 'A scalable neural hardware execution engine optimizing matrix multiplications.',
      claims: [{ id: 1, type: 'Independent', text: 'A neural hardware accelerator comprising execution units...' }]
    };

    const reportContent = `=====================================================================
PATENTMIND AI ENTERPRISE DOSSIER REPORT
=====================================================================
PATENT NUMBER : ${targetPatent.number}
TITLE         : ${targetPatent.title}
ASSIGNEE      : ${targetPatent.assignee}
INVENTORS     : ${targetPatent.inventors}
FILING DATE   : ${targetPatent.filingDate}
CPC CODE      : ${targetPatent.cpcCode}
MATCH SCORE   : ${targetPatent.similarityScore}%
FTO RISK      : ${targetPatent.infringementRiskScore}%

---------------------------------------------------------------------
1. ABSTRACT & CORE TECHNICAL CONTRIBUTION
---------------------------------------------------------------------
${targetPatent.abstract}

---------------------------------------------------------------------
2. PATENT CLAIMS HIERARCHY
---------------------------------------------------------------------
${(targetPatent.claims || []).map(c => `${c.type || 'Claim'} #${c.id}: ${c.text}`).join('\n\n')}
...
=====================================================================`;
    ...
    a.href = url;
    a.download = `Patent_${targetPatent.number}_Dossier.txt`;
  ...
  const patent = {
    ...
    similarityScore: Number((91.5 + (patentNumber ? Math.abs([...patentNumber].reduce((acc, c) => acc + c.charCodeAt(0), 0)) % 65 / 10 : 3.2)).toFixed(1)),
        ...
        <div className="wrangler-card p-6 flex items-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-300">
              Matches your active research vector embeddings across 12 claims with {patent.similarityScore}% cosine confidence.
            </p>
          </div>
        </div>
